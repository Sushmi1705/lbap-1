...
const productImages = [
  {
    src: '/automotive_fabrication_components.png',
    alt: 'Automotive Fabrication – Sheet Metal Assembly',
  },
  {
    src: '/automotive_fabrication_components.png',
    alt: 'Deep Drawn Sheet Metal Components',
  },
];

const features = ['Dimensional Accuracy', 'Corrosion Free', 'Durability'];

const springFast = { type: 'spring', stiffness: 220, damping: 22 };

const AutomotiveFabricationComponents = () => {
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-['Outfit']">
      ...
      <section className="relative bg-[#03072c] py-14 overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#FF5C00]/10 rounded-full blur-[100px] pointer-events-none" />
        <div className="container-custom relative z-10">
          ...
          >
            <div className="flex items-center gap-2 text-slate-400 text-xs font-semibold uppercase tracking-widest mb-4">
              ...
              <span className="text-[#00A7FF]">Automotive Fabrication Components</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight leading-tight">
              Automotive <span className="text-[#FF5C00]">Fabrication Components</span>
            </h1>
            <p className="text-slate-400 mt-3 text-sm font-medium max-w-xl">
              Fabricated Metal Parts and Automotive Sheet Metal Components that meet the specific requirements of automobile manufacturers.
            </p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            >
              ...
              <div className="relative rounded-2xl overflow-hidden bg-[#0a1a3a] shadow-lg mb-8 aspect-[16/7]">
                ...
                <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white/40 transition-all border border-white/20" aria-label="Previous">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                </button>
                <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white/40 transition-all border border-white/20" aria-label="Next">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                </button>
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                  {productImages.map((_, i) => (
                    <button key={i} onClick={() => setActiveSlide(i)} className={`h-2 rounded-full transition-all ${i === activeSlide ? 'bg-[#FF5C00] w-5' : 'bg-white/50 w-2'}`} aria-label={`Slide ${i + 1}`} />
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
                <div className="p-8 md:p-10">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                      <span className="text-[#FF5C00] text-xs font-bold uppercase tracking-widest mb-2 block">Product Overview</span>
                  ...
                  <div className="w-16 h-1 bg-[#FF5C00] mb-6 rounded-full" />

                  <p className="text-slate-600 text-sm leading-relaxed font-medium mb-6">
                    We are engaged in offering a wide range of Fabricated Metal Parts, Automotive Sheet Metal Components, which meet the specific requirements of automobile manufacturers. The range we offer include deep drawn sheet metal components, stamped sheet metal parts, sheet metal assembly and others that successfully meet the demands of critical profiles &amp; accuracy. Our range of Automotive Sheet Metal Components is available in various types and we offer at most affordable prices.
                  </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                    ...
                    <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">Precision Built</span>
                    </div>
                    <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
                {sidebarCategories.filter(c => c.path !== '/products/automotive-fabrication-components').map((cat) => (
                  <NavLink key={cat.path} to={cat.path} className="group flex items-center gap-3 p-4 bg-white border border-slate-200/60 rounded-xl hover:border-[#00A7FF]/40 hover:shadow-md transition-all duration-300">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1 text-[10px] text-slate-400 font-medium mt-0.5 group-hover:text-[#00A7FF]/70 transition-colors">View <ArrowRight className="w-3 h-3" /></div>
                    </div>
                  </NavLink>
                ))}
              </div>
      ...
      <AnimatePresence>
        {enquiryOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setEnquiryOpen(false)}>
            <motion.div initial={{ scale: 0.9, y: 24, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 24, opacity: 0 }} transition={springFast} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8" onClick={(e) => e.stopPropagation()}>
              <h3 className="text-xl font-black text-slate-900 mb-1">Send an Enquiry</h3>
              <p className="text-slate-500 text-sm mb-6">Regarding: <span className="font-bold text-[#FF5C00]">Automotive Fabrication Components</span></p>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setEnquiryOpen(false)} className="flex-1 py-3 border border-slate-200 rounded-lg text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors">Cancel</button>
                <button className="flex-1 py-3 bg-[#FF5C00] hover:bg-[#00A7FF] text-white rounded-lg text-sm font-bold uppercase tracking-wider transition-colors">Submit Enquiry</button>
              </div>
            </motion.div>
          </motion.div>
        )}
...
export default AutomotiveFabricationComponents;
